...
int graph_io::writeGraph(graph_access & G, const std::string & filename, bool halveEdges) {
        std::ofstream f(filename.c_str());
        f << G.number_of_nodes() <<  " ";
        if (halveEdges) {
                f <<  G.number_of_edges()/2 << std::endl;
        } else {
                f <<  G.number_of_edges() << std::endl;
        }
...
int graph_io::readDiGraph(graph_access & G, const std::string & filename) {
        std::string line;
