...
inline NodeID graph_access::unsafe_getEdgeTarget(EdgeID edge){
    return graphref->m_edges[edge].target;
}
...
inline void graph_access::buildReverseGraph(graph_access & output) {
         basicGraph& ref = *graphref;
        std::size_t n = number_of_nodes();
        std::size_t m = number_of_edges();

        std::vector<std::vector<NodeID>> nodes(n);

        
        // store in edges for each vertex
        forall_nodes(ref, node) {
                forall_out_edges(ref, e, node) {
                        nodes[unsafe_getEdgeTarget(e)].push_back(node);                        
                } endfor
        } endfor

        // todo: maybe use weights of the input graph
        NodeWeight weight = 1;
        EdgeWeight edge_weight = 1;

        std::size_t node_counter = 0;
        std::size_t edge_counter = 0;
        output.start_construction(n, m);

        for (auto neighbours : nodes) {
                NodeID node = output.new_node(); node_counter++;
                output.setPartitionIndex(node, 0);
                output.setNodeWeight(node, weight);
        }

        for (std::size_t u = 0; u < node_counter; u++) {
                for (auto v : nodes[u]) {
                        edge_counter++;
                        EdgeID e = output.new_edge(u, v);
                        output.setEdgeWeight(e, edge_weight);
                }
        }

        output.finish_construction();
}
